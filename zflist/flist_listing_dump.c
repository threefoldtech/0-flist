    ...
    for(int i = 0; i < capn_len(root->dir.contents); i++) {
        ...
        if(inode.attributes_which == Inode_attributes_file) {
            ...
            for(int i = 0; i < capn_len(file.blocks); i++) {
                ...
                char *hashstr = libflist_hashhex((unsigned char *) block.hash.p.data, block.hash.p.len);
                char *keystr = libflist_hashhex((unsigned char *) block.key.p.data, block.key.p.len);
